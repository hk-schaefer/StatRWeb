...
```{r}
load("~/R/StatRWeb/Data/gss.Rdata")
```
\
\
\
...
- There is a *variability in sample size between the years*. Results from sample statistics cannot be generalized over the whole time span from 1972 to 2012. Instead it *should be generalized only to a respective year*. All sample sizes are above 1000 and, therefore, *can be generalized to the population*.\
...
\
**The sample statistics can be generalized to the US population, but we may see a certain voluntary bias** due to the sampling methodology.\
\
\
...
Five meaningful research questions were chosen from the GSS data set to cover the  learning objective of this course:  
...
\
\
**Common names for statistical variables:**\

- mue: true mean of population\
- s: (sigma) true standard deviation of population\
...
- moe  : margin of error\
- SF   : Success-Failure condition\
- Z    : value from z-statistics (normal distribution)\
- T    : value from t-statistics (t - distribution)\
- F    : value from F-statistics (F - distribution)\
- X2   : value from chi-square statistics (Chi-square distribution)
- dof  : degree of freedom (e.g. n-1)\
...
Alternative hypothesis (Ha): There is strong evidence that the American population approves suicide in case of incurable disease.\
...
Alternative hypothesis (Ha): There is strong evidence of a difference in belief between White and Non-white American population that hard work leads to success in life.\
...
**Create a 2-way-table (contingency table) for the observations**
...
- The test statistics is comparing the observed values/proportions with expected values/proportions. The expected values are calculated from marginal proportions.\
...
Expected values (2-way table)
```{r}
# Print expected values:  Calculated from obs. values by independence rule 
# P(A & B) = P(A) * P(B)
model$expected
```
\
...
\
**Conclusion: **
The X-squared value is large (62.2) and the p-value is nearly zero. We can, therefore, reject H0 hypothesis. I.e. the political party preference is depends on the degree of education.\
...
Alternative hypothesis (Ha): There is strong evidence that Americans with more years in education think that the government spending for space exploration is too little.\
...
\
**Conclusion: **
The data show strong evidence that Americans with more years in education think that the government spending for space exploration is too little. The p-value is < 0.0001. I.e. the probability to observe a mean difference of 1.61 or more if in fact H0 is true is less than 0.01%.\
...
- The ANOVA test delivers a p-value of 0.004 which is less than alpha 0.05. Therefore, we reject the Null Hypothesis. I.e. there is strong evidence that the average number of children per class differs at least in one pair.\
...
- Only in the pairs "Working Class - Lower Class" and "Middle Class - Lower Class" we do have strong evidence that there is a difference in the average number of children per class.\
\
\
\
\
